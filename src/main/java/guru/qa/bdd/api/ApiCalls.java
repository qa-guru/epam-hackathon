...
import guru.qa.bdd.utils.CookieUtils;
...
import static guru.qa.bdd.api.HttpClientFactory.BASE_URL;
...
public class ApiCalls {
    ...
    public void apiLogin(String email, String password) throws IOException, InterruptedException {
        ...
        HttpRequest request = HttpRequest.newBuilder()
                ...
                .header("Cookie", CookieUtils.parseCookieList(cookiesVeryNeeded.get()))
                .POST(requestBody)
                .build();

        HttpResponse<String> response = httpClient.send(request, HttpResponse.BodyHandlers.ofString());
        List<String> cookies = response.headers().allValues("Set-Cookie");
        for (Cookie cookie : CookieUtils.parseCookie(cookies)) {
